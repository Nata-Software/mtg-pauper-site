...
import { DeckMatchLog } from "@/components/DeckMatchLog";
import { getCardArtUrls } from "@/lib/scryfall";
import { monthsAgoISO, toISODate } from "@/lib/dates";
import { t, type Locale, type TranslationKey } from "@/lib/i18n";
import { getLocale } from "@/lib/i18n.server";
import { getDeckMatchLog, getMetagameData, listStores } from "@/lib/queries";
  ...
  return (
    <div>
      ...
      {selectedDeck ? (
        <DeckMatchLog
          deck={selectedDeck}
          rows={await getDeckMatchLog({ store, from, to }, selectedDeck)}
          backHref={baseHref}
          locale={locale}
        />
      ) : (
        <MetagameGridSection store={store} from={from} to={to} baseHref={baseHref} locale={locale} />
      )}
    </div>
  );
}
...
async function MetagameGridSection({
  ...
  return (
    <MetagameGrid decks={decks} cardArt={cardArt} baseHref={baseHref} locale={locale} />
  );
}
